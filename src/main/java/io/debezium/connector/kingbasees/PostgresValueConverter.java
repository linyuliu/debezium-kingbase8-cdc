...
import io.debezium.connector.kingbasees.PostgresConnectorConfig.HStoreHandlingMode;
import io.debezium.connector.kingbasees.PostgresConnectorConfig.IntervalHandlingMode;
import io.debezium.connector.kingbasees.data.Ltree;
...
public class PostgresValueConverter extends JdbcValueConverters {
    ...
    public static final Instant POSITIVE_INFINITY_INSTANT = Conversions.toInstantFromMicros(KBStatement.DATE_POSITIVE_INFINITY);
    ...
    public static final OffsetDateTime POSITIVE_INFINITY_OFFSET_DATE_TIME = OffsetDateTime.ofInstant(Conversions.toInstantFromMillis(KBStatement.DATE_POSITIVE_INFINITY),
            ZoneOffset.UTC);

    public static final Timestamp NEGATIVE_INFINITY_TIMESTAMP = new Timestamp(KBStatement.DATE_NEGATIVE_INFINITY);
    public static final Instant NEGATIVE_INFINITY_INSTANT = Conversions.toInstantFromMicros(KBStatement.DATE_NEGATIVE_INFINITY);
    ...
    public static final OffsetDateTime NEGATIVE_INFINITY_OFFSET_DATE_TIME = OffsetDateTime.ofInstant(Conversions.toInstantFromMillis(KBStatement.DATE_NEGATIVE_INFINITY),
            ZoneOffset.UTC);
    ...
    private Object convertLtree(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, "", r -> {
            ...
            else if (data instanceof KBobject) {
    ...
    private Object convertLtreeArray(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, Collections.emptyList(), r -> {
            ...
            else if (data instanceof KbArray) {
                KbArray KbArray = (KbArray) data;
                try {
                    Object[] array = (Object[]) KbArray.getArray();
                    ...
                    logger.error("Failed to parse KbArray: " + KbArray, e);
                }
            }
        });
    }
     ...
     * com.kingbase8.core.Encoding))}, but the String based representation
     */
    ...
    protected Object convertBits(Column column, Field fieldDefn, Object data, int numBytes) {
        if (data instanceof KBobject) {
            // returned by the JDBC driver
            data = ((KBobject) data).getValue();
        }
        if (data instanceof String) {
    ...
    protected Object convertInterval(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, NumberConversions.LONG_FALSE, (r) -> {
            ...
            if (data instanceof KBInterval) {
                final KBInterval interval = (KBInterval) data;
                if (intervalMode == IntervalHandlingMode.STRING) {
                    r.deliver(
    ...
    protected Object convertCitext(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, "", (r) -> {
            if (data instanceof byte[]) {
                ...
                r.deliver(((KBobject) data).getValue());
            }
        });
    }
    ...
    protected Object convertPoint(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, Point.createValue(fieldDefn.schema(), 0, 0), (r) -> {
            final Schema schema = fieldDefn.schema();
            if (data instanceof KBpoint) {
                KBpoint KBpoint = (KBpoint) data;
                r.deliver(Point.createValue(schema, KBpoint.x, KBpoint.y));
            }
            else if (data instanceof String) {
                String dataString = data.toString();
                try {
                    KBpoint KBpoint = new KBpoint(dataString);
                    r.deliver(Point.createValue(schema, KBpoint.x, KBpoint.y));
                }
                catch (SQLException e) {
                    logger.warn("Error converting the string '{}' to a KBpoint type for the column '{}'", dataString, column);
                }
            }
    ...
    protected Object convertArray(Column column, Field fieldDefn, PostgresType elementType, ValueConverter elementConverter, Object data) {
        return convertValue(column, fieldDefn, data, Collections.emptyList(), (r) -> {
            if (data instanceof List) {
                ...
                try {
                    final Object[] values = (Object[]) ((KbArray) data).getArray();
        ...
        switch (elementType.getOid()) {
            case PgOid.TIMETZ: {
                if (value instanceof java.sql.Time) {
                    ...
                    // Daylight savings gets applied by KbArray, need to account for that here
    ...
    /**
     * Extracts a value from a KBobject .
     *
    ...
    protected Object convertBinaryToBytes(Column column, Field fieldDefn, Object data) {
        ...
        if (data instanceof KbArray) {
            data = ((KbArray) data).toString();
        }
        return super.convertBinaryToBytes(column, fieldDefn, (data instanceof KBobject) ? ((KBobject) data).getValue() : data);
    }

    @Override
    protected Object convertBinaryToBase64(Column column, Field fieldDefn, Object data) {
        return super.convertBinaryToBase64(column, fieldDefn, (data instanceof KBobject) ? ((KBobject) data).getValue() : data);
    }

    @Override
    protected Object convertBinaryToHex(Column column, Field fieldDefn, Object data) {
        return super.convertBinaryToHex(column, fieldDefn, (data instanceof KBobject) ? ((KBobject) data).getValue() : data);
    }
    ...
    protected Object convertTinyInt(Column column, Field fieldDefn, Object data) {
        final Object normalized = normalizeNumericData(data);
        return convertValue(column, fieldDefn, normalized, (byte) 0, r -> {
            if (normalized instanceof Number) {
                r.deliver(((Number) normalized).byteValue());
            }
        });
    }
    ...
    protected Object convertSmallInt(Column column, Field fieldDefn, Object data) {
        Object normalized = normalizeNumericData(data);
        if (normalized instanceof String) {
            return null;
        }
        if (normalized instanceof Number && !(normalized instanceof Short)) {
            normalized = ((Number) normalized).shortValue();
        }
        return super.convertSmallInt(column, fieldDefn, normalized);
    }
    ...
    protected Object convertInteger(Column column, Field fieldDefn, Object data) {
        ...
        if (normalized instanceof Number && !(normalized instanceof Integer)) {
            normalized = ((Number) normalized).intValue();
        }
        return super.convertInteger(column, fieldDefn, normalized);
    }
    ...
    protected Object convertBigInt(Column column, Field fieldDefn, Object data) {
        ...
        if (normalized instanceof Number && !(normalized instanceof Long)) {
            normalized = ((Number) normalized).longValue();
        }
        return super.convertBigInt(column, fieldDefn, normalized);
    }
    ...
    private Object normalizeNumericData(Object data) {
        if (data instanceof KBobject) {
            data = ((KBobject) data).getValue();
        }
        if (data instanceof String) {
