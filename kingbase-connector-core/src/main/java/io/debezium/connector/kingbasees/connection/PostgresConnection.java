...
import io.debezium.connector.kingbasees.spi.SlotState;
...
public class PostgresConnection extends JdbcConnection {
    ...
    private static final String URL_PATTERN = "jdbc:kingbase8://${" + JdbcConfiguration.HOSTNAME + "}:${"
    ...
    protected Optional<ColumnEditor> readTableColumn(ResultSet columnMetadata, TableId tableId, Tables.ColumnNameFilter columnFilter) throws SQLException {
        ...
        if (columnFilter == null || columnFilter.matches(tableId.catalog(), tableId.schema(), tableId.table(), columnName)) {
            ...
            final PostgresType rootType = nativeType.getRootType();
            column.nativeType(rootType.getOid());
            column.jdbcType(rootType.getJdbcId());

            if (rootType == PostgresType.UNKNOWN || rootType.getJdbcId() == Integer.MIN_VALUE) {
                final int fallbackJdbcType = fallbackJdbcTypeForUnknown(column.typeName());
                column.jdbcType(fallbackJdbcType);
                LOGGER.warn("Type '{}' was not resolved by TypeRegistry; fallback JDBC type {} (VARCHAR/string-friendly fallback) will be used",
                        column.typeName(), fallbackJdbcType);
            }
    ...
    private static int fallbackJdbcTypeForUnknown(String rawTypeName) {
        String normalized = TypeRegistry.normalizeTypeName(rawTypeName).toLowerCase(Locale.ROOT);
        switch (normalized) {
            case "int2":
